...
TABLES = [
    ...
    # Now uses negative exclusion — all NC campaigns not carrying any of the specific bucket labels.
    ('NC Non-Testing Campaigns', {'Customer Type': 'NC', 'exclude_labels': NC_SPECIFIC_LABELS}, 'vbb'),
]
...
def aggregate(df, filters):
    ...
    for col, val in filters.items():
        # FIX: handle 'exclude_labels' — exclude any row whose label contains
        # any of the listed strings. Used to define NC Non-Testing as a
        # negative exclusion rather than a positive label match.
        if col == 'exclude_labels':
            ...
            # Column not present in export — skip silently rather than
            # returning all rows (previously caused MSFT CBB NB to over-count)
            continue
        elif col == labels_col:
